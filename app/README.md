# app — o executável `spaceflight` (ADR-0009)

A apresentação entra aqui **apenas como consumidora** do `spaceflight_core`.
Nada nesta pasta calcula física — nem o C++ da apresentação, nem os shaders.
Até o Milestone 8 esta camada era um projeto Godot com uma GDExtension; o
[ADR-0009](../docs/adr/0009-render-stack.md) diz por que deixou de ser.

```
app/
├── session/        FlightSession: estado, relógio, autopiloto, planejador (numa
│                   thread), StarSky (catálogo + tabela de Planck), transfer_bridge
├── presentation/   tudo o que se vê e se comanda, SEM GPU e SEM SDL:
│   ├── flight_app       o orquestrador: constrói tudo e encaminha a entrada
│   ├── input_actions    TODO o mapeamento de teclas, numa tabela
│   ├── flight_controls  o que o piloto comanda
│   ├── camera_rig       as DUAS câmeras e os cinco modos
│   ├── scene/           nave, pluma, jatos, cockpit, planetas, estrelas, materiais
│   ├── instruments/     as quatro telas do cockpit e o mapa orbital
│   ├── ui/              painel de missão e HUD técnico (o texto; o desenho é do ImGui)
│   ├── headless_driver  a verificação sem tela
│   └── shot_director    os roteiros fotografados do M7 e do M8
├── gfx/            o renderizador SDL_GPU e o renderer do ImGui
├── shaders/        GLSL 450: estrelas, corpos, casco, composite, UI
├── platform/       caminhos de dados, áudio e teclado SDL
├── ui/             os painéis ImGui (HUD, mapa, missão, pausa, ajuda)
└── main.cpp        o laço: janela ou fora da tela, headless, capturas
```

`presentation/` não inclui SDL nem nada de GPU, e é por isso que os testes
`presentation.*` rodam em qualquer máquina: desenham num `Canvas` que grava as
chamadas (`canvas.hpp`) em vez de pintar pixels.
...
cmake --build build -j
./build/bin/spaceflight                 # janela 1440 x 900
```

SDL3, Dear ImGui, stb, glslang e SPIRV-Cross vêm por `FetchContent`, fixados por
SHA-256 (`cmake/third_party.cmake`). Os shaders são compilados no build
(GLSL → SPIR-V → MSL) e embutidos no binário. Backends: Metal no macOS, Vulkan no
Linux; Direct3D 12 no Windows está previsto e não foi compilado.

Os dados são procurados, nesta ordem: variável de ambiente
(`SPACEFLIGHT_ASSET_DIR`, `SPACEFLIGHT_KERNEL_DIR`, `SPACEFLIGHT_CATALOG_DIR`),
ao lado do executável, em `../share/spaceflight/`, e por fim no fonte. Então
...
é um jogo que se move ou se zipa inteiro.

| Opção | O que faz |
|---|---|
| `--headless [--frames N] [--destination Moon\|Mars]` | sem GPU; imprime a leitura técnica; voa a missão |
| `--shots m7\|m8 DIR [--stop N]` | o roteiro do milestone, fotografado fora da tela |
| `--capture DIR` | a escada de β do Milestone 5, uma foto por degrau |
| `--screenshot FILE [--frames N]` | um quadro, fora da tela |
| `--dump-controls [DIR]` | regenera `controls.md` e `controls.json` |
| `--resolution WxH`, `--window`, `--gpu-debug` | tamanho, forçar janela, validação do driver |
| `--kernels DIR`, `--assets DIR`, `--catalogue FILE` | onde estão os dados |

As corridas roteirizadas renderizam **numa textura**, sem janela, com passo fixo
de 1/60 s: a mesma imagem em qualquer máquina com GPU, e nenhuma tela necessária.

## Verificação headless

```
$ ./build/bin/spaceflight --headless       # 450 quadros, ~0,5 s
```
...
PROJECTION at beta = 0.0896 (the ship is NOT at this speed; the state is untouched)
  forward cone 84.859 deg holds 4311 stars (49.07 %)
  doppler      0.9141 astern .. 1.0940 ahead
  5800 K star  1.476 x ahead, 0.6549 x astern  (visible band, not bolometric)

PROJECTION at beta = 0.9048 ...
  forward cone 25.204 deg holds 4311 stars (49.07 %)
  doppler      0.2236 astern .. 4.4731 ahead
  5800 K star  51.48 x ahead, 3.295e-7 x astern  (visible band, not bolometric)

PROJECTION at beta = 0.9900 ...
  forward cone  8.110 deg holds 4311 stars (49.07 %)
  doppler      0.0709 astern .. 14.1067 ahead
  5800 K star  238.8 x ahead, 2.199e-23 x astern  (visible band, not bolometric)
```
...
* **4 311 estrelas nos três**. Não é coincidência nem arredondamento: a aberração
...
(`docs/physics/b-plane.md`), a inserção. A apresentação carrega números e mostra-os.

A busca roda numa **thread** (`FlightSession`), e o quadro continua: o painel
mostra os candidatos a subir. O plano só é instalado no estado pelo quadro, nunca
pelo worker. Sem tela, a missão inteira é uma linha:

```
$ ./build/bin/spaceflight --headless --destination Moon     # ~13 s
about Moon      CAPTURED   1840.1 km at 1631.1 m/s
  orbit        96.7 x 103.2 km altitude, e 0.0018, i 19.50 deg, 117.8 min
```
...
Agora não há palpite. `Interface::draw_debug` **pergunta à fonte** a altura de
uma linha e a largura da linha mais comprida, e desce o tamanho até que as duas
caibam. No máximo treze iterações, e só quando
...
Os números também perderam ruído: a primeira versão imprimia tudo o que o
...
`--frames N` conta **quadros**, e o passo é fixo em 1/60 s. A versão do
Milestone 2 imprimia a cada segundo de relógio de parede, o que fazia a
verificação depender da velocidade da máquina. Agora é a cada 150 quadros, e o
mesmo comando dá a mesma saída em qualquer lugar.
...
```
$ ./build/bin/spaceflight --headless --frames 6000
[headless] commanded PROGRADE
[headless] throttle 100% at 2.958 deg of pointing error
[headless] CRUISE -- exhaust 0.5 c, budget 0.9048 c
```

⚠️ **Demora minutos**, e de propósito: a partir do quadro ~500 cada quadro é
10⁷ vezes 1/60 s de voo **propulsado**, e o integrador não pula uma queima. Os
450 quadros do padrão param antes disso.

## Controles

A tabela completa está em [`docs/gameplay/controls.md`](../docs/gameplay/controls.md),
**gerada** a partir de `presentation/input_actions.cpp` por `scripts/dump_controls.sh` --
...
é pequeno, como tem de ser. O quarto é o polo da órbita, onde o `looking_at`
...
câmera e é medido na apresentação; o `D` **não** é: vem de `core/relativity/optics.hpp`
pelo `StarSky::doppler_in_direction()`, porque é física.
...
`spaceflight_core` e as suítes `unit|integration|scientific|regression` compilam
e passam **sem esta pasta** (`-DSPACEFLIGHT_BUILD_APP=OFF`) — é assim que se
verifica que a separação continua real.

Dentro da apresentação:

* a cena recebe `SimulationSnapshot`, nunca ponteiro para o integrador;
* `double → float` acontece uma única vez, depois do `RenderTransform` subtrair a
  origem da câmera (`docs/architecture/rendering.md`);
* nenhuma exceção atravessa o quadro — toda entrada da sessão passa por um guard
  que converte em erro logado e `false`;
