...
//
// The cockpit supplies parameters, picks a destination, starts the planning, shows the
...
// the user interface.
//
// Plain core types only, so the bridge can be read, reasoned about and
// exercised without a window: tests/scientific/test_planner_equivalence.cpp
// links it and nothing else from app/.
...
// flight session owns the lifetimes and this struct lives for one call.
